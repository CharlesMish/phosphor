...
describe("phase-coherent CYCLE morph runtime", () => {
  ...
  it("does not restart a ramp on repeated flat Motion ticks", () => {
    withTestEngine((engine, context) => {
      ...
      const b = generatePreset("triangle");
      engine.setCycleMorph(a, b, 0);
      engine.noteOn(60);
      engine.setCycleMorph(a, b, 0.4);
      const gain = context.oscillators[3]?.connections[0];
      assert.ok(gain instanceof TestGainNode);
      const count = gain.gain.linearRampCalls.length;
      context.currentTime = 0.01;
      engine.setCycleMorph(a, b, 0.4);
      assert.equal(gain.gain.linearRampCalls.length, count);
    });
  });

  it("retargets interrupted ramps from the actual blend, including loop wraps", () => {
    withTestEngine((engine, context) => {
      const a = generatePreset("sine");
      const b = generatePreset("triangle");
      engine.setCycleMorph(a, b, 0);
      engine.noteOn(60);
      const gainA = context.oscillators[2]?.connections[0];
      const gainB = context.oscillators[3]?.connections[0];
      assert.ok(gainA instanceof TestGainNode);
      assert.ok(gainB instanceof TestGainNode);
      engine.setCycleMorph(a, b, 1);
      context.currentTime = CYCLE_MORPH_RAMP_SECONDS / 2;
      // The mock deliberately reports the future target as .value. The engine
      // must reconstruct 0.5 instead of jumping to that value on interruption.
      assert.equal(gainB.gain.value, 1);
      engine.setCycleMorph(a, b, 0);
      assert.equal(gainA.gain.setValueCalls.at(-1), 0.5);
      assert.equal(gainB.gain.setValueCalls.at(-1), 0.5);
      assert.equal(gainB.gain.linearRampCalls.at(-1)?.value, 0);
      assert.equal(context.oscillators.length, 4);
    });
  });

  it("cancels queued direct drawing when morph takes control", () => {
    withTestEngine((engine, context, timers) => {
      const a = generatePreset("sine");
      const b = generatePreset("square");
      engine.setWaveform(a, true);
      engine.noteOn(60);
      engine.setWaveform(b);
      assert.equal(timers.size, 1);
      engine.setCycleMorph(a, b, 0.3);
  ...
  it("cancels a queued waveform flush when a gesture finishes immediately", () => {
    withTestEngine((engine, _context, timers) => {
      engine.setWaveform(generatePreset("sine"), true);
      engine.setWaveform(generatePreset("square"));
      assert.equal(timers.size, 1);
  ...
  it("crossfades once into the morph pair for an already-held note", () => {
    withTestEngine((engine, context) => {
      const sine = generatePreset("sine");
      const triangle = generatePreset("triangle");
      engine.setWaveform(sine, true);
