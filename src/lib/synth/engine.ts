...
export class SynthEngine {
  ...
  setCycleMorph(
    ...
    // A cancelled direct-drawing update must not resurrect after morphing.
    this.pendingSamples = null;
    if (runtime && !endpointsChanged && runtime.value === u) return;
    if (endpointsChanged) {
  ...
  private createCycleMorphSource(
    ...
    return {
      ...
      ramp: { from: runtime.value, to: runtime.value, start: now, end: now },
    };
  }
  ...
  private rampCycleMorphSource(
    ...
    const { from, to, start, end } = source.ramp;
    const progress = end <= start
      ? 1
      : Math.min(1, Math.max(0, (now - start) / (end - start)));
    const current = from + (to - from) * progress;
    // Derive the held value from our ramp rather than AudioParam.value, whose
    // treatment of automation differs from a simple last-assigned value.
    // This also works in browsers without cancelAndHoldAtTime.
    source.gainA.gain.cancelScheduledValues(now);
    source.gainB.gain.cancelScheduledValues(now);
    source.gainA.gain.setValueAtTime(1 - current, now);
    source.gainB.gain.setValueAtTime(current, now);
    source.ramp = {
      from: current,
      to: value,
      start: now,
      end: now + CYCLE_MORPH_RAMP_SECONDS,
    };
